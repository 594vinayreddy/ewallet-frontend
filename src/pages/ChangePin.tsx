...
import { updatePin } from '../api/walletApi';
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .page-wrap{min-height:100vh;background:radial-gradient(ellipse at 20% 20%,rgba(79,124,255,0.07) 0%,transparent 50%),radial-gradient(ellipse at 80% 80%,rgba(155,109,255,0.07) 0%,transparent 50%),#080b12;display:flex;flex-direction:column}
  ...
  .card-icon{width:54px;height:54px;border-radius:16px;background:rgba(79,124,255,0.12);display:flex;align-items:center;justify-content:center;font-size:1.5rem;margin-bottom:20px}
  ...
  .pin-box.active{border-color:#4f7cff;background:rgba(79,124,255,0.07);box-shadow:0 0 0 3px rgba(79,124,255,0.14)}
  .pin-box.filled{border-color:rgba(79,124,255,0.4)}
  ...
  .step-dot.active{background:#4f7cff}
  .step-dot.done{background:rgba(79,124,255,0.45)}

  .step-label{font-size:.75rem;color:rgba(245,247,255,0.4);margin-bottom:20px;font-weight:500}
  .step-label span{color:#4f7cff;font-weight:600}

  .submit-btn{width:100%;padding:14px;border:none;border-radius:13px;background:linear-gradient(135deg,#4f7cff,#9b6dff);color:#fff;font-family:'Syne',sans-serif;font-size:.94rem;font-weight:700;letter-spacing:.05em;cursor:pointer;box-shadow:0 6px 30px rgba(79,124,255,0.3);transition:transform .18s,box-shadow .18s;margin-top:20px}
  .submit-btn:hover{transform:translateY(-2px);box-shadow:0 10px 40px rgba(79,124,255,0.4)}
        ...
        {Array.from({ length: PIN_LENGTH }).map((_, i) => {
          ...
          return (
            <div
              ...
              style={filled || active ? { borderColor: accentColor, color: accentColor } : {}}
            >
              {filled ? '•' : ''}
            </div>
          );
...
const STEPS: Step[] = ['old', 'new', 'confirm'];

const STEP_META: Record<Step, { label: string; sublabel: string; stepNum: number }> = {
  old:     { label: 'Current PIN',     sublabel: 'Enter your current 4-digit PIN.',           stepNum: 1 },
  new:     { label: 'New PIN',         sublabel: 'Choose a new 4-digit PIN for your wallet.', stepNum: 2 },
  confirm: { label: 'Confirm New PIN', sublabel: 'Re-enter your new PIN to confirm.',          stepNum: 3 },
};
...
export default function ChangePin() {
  ...
  const setCurrentValue = step === 'old' ? setOldPin : step === 'new' ? setNewPin : setConfirmPin;

  // Auto-advance when PIN is fully entered
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await updatePin({ userId: user.id, oldPin, newPin });
  ...
  return (
    <div className="page-wrap">
      <div className="topbar">
        ...
        <span className="page-title">Change PIN</span>
      </div>
      ...
      <div className="center">
        <div className="card">
          ...
          <div className="field">
            <label>{label}</label>
            <PinInput
              key={step}            // remount on step change to reset focus
              value={currentValue}
          ...
          {loading && (
            <button className="submit-btn" disabled>Updating PIN…</button>
          )}
